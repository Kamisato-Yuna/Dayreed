...
`retentionDays` 默认 30，范围 1...3650。`start()`、设置变化及成功采样后应用保留策略，保留当前日历日及之前 days-1 天。`applyRetention(days:now:calendar:)` 支持明确时区，按日历计算，包含夏令时日期边界。关闭来源不会立即删除此前记录，已有内容依然遵循保留设置。
